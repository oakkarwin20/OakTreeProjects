...
//----------------------------------------------------------------------------------------------------------------------
GameMode_CCD_3D::GameMode_CCD_3D()
{
}
...
//----------------------------------------------------------------------------------------------------------------------
void GameMode_CCD_3D::Update( float deltaSeconds )
{	
	// Update core systems
...
//----------------------------------------------------------------------------------------------------------------------
void GameMode_CCD_3D::Render() const
{
	RenderWorldObjects();
	RenderUIObjects();
}
...
//----------------------------------------------------------------------------------------------------------------------
void GameMode_CCD_3D::Shutdown()
{
}


//----------------------------------------------------------------------------------------------------------------------
void GameMode_CCD_3D::UpdateDebugKeys()
{
...
//----------------------------------------------------------------------------------------------------------------------
void GameMode_CCD_3D::UpdateCameraInput( float deltaSeconds )
{
...
//----------------------------------------------------------------------------------------------------------------------
void GameMode_CCD_3D::UpdateGameMode3DCamera()
{
	//----------------------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------------------------
void GameMode_CCD_3D::RenderWorldObjects() const
{
	//----------------------------------------------------------------------------------------------------------------------
	// Begin World Camera
	//----------------------------------------------------------------------------------------------------------------------
	...
	g_theApp->m_textFont->AddVertsForText3D( textVerts, textOrigin, iBasis, jBasis, 25.0f, "CCD 3D!", Rgba8::GREEN );
	...
	{
		m_creatureCCD->RenderMesh( g_theRenderer, nullptr, g_litShader_PCUTBN );
	}
	else
	{
		m_creatureCCD->Render( creatureVerts, Rgba8::WHITE, Rgba8::YELLOW );
	}
	m_ikChain_CCD->DebugDrawTarget( verts, 2.5f, Rgba8::MAGENTA, true, 8.0f );
	...
	if ( g_debugBasis_F3 )
	{
		m_ikChain_CCD->DebugDrawJoints_IJK( verts, 0.3f, 4.0f );
	}
	if ( g_debugText_F4 )
	{
	...
	if ( g_debugAngles_F5 )
	{
		m_ikChain_CCD->DebugDrawConstraints_YPR( verts_NoBackfaceCull, 3.0f );
	}
...
//----------------------------------------------------------------------------------------------------------------------
void GameMode_CCD_3D::RenderUIObjects() const
{
	//----------------------------------------------------------------------------------------------------------------------
	// Begin UI Camera
	...
	if ( g_debugFreeFly_F1 )
	{
		gameInfo = Stringf( "F1 (Toggle camera/player) Camera || Mode (F6/F7 for prev/next) || CCD (3D)" );
	}						
	else
	{
		gameInfo = Stringf( "F1 (Toggle camera/player) Player || Mode (F6/F7 for prev/next) || CCD (3D)" );
	}
	...
	if ( g_debugRenderRaycast_F2 )
	{
		g_theApp->m_textFont->AddVertsForTextInBox2D( textVerts, textbox1, cellHeight, gameInfo, textColor_darkRed, 0.75f, alignment, TextDrawMode::SHRINK_TO_FIT );
	...
	if ( g_debugText_F4 )
	{
		...
		g_theApp->m_textFont->AddVertsForTextInBox2D(	textVerts, textbox1, cellHeight, 
			...
			textColor_darkGreen, 0.75f, Vec2( 1.0f, textAlignmentY -= textHeight * 2.0f ), TextDrawMode::SHRINK_TO_FIT );
		//----------------------------------------------------------------------------------------------------------------------
		// Joint 1
		...
		if ( m_ikChain_CCD->m_jointList.size() >= 6 )
		{
			g_theApp->m_textFont->AddVertsForTextInBox2D(	textVerts, textbox1, cellHeight, 
				...
				Rgba8::YELLOW, 0.75f, Vec2( 1.0f, textAlignmentY -= textHeight ), TextDrawMode::SHRINK_TO_FIT );		
		}
		//----------------------------------------------------------------------------------------------------------------------
		// Joint 6
...
//----------------------------------------------------------------------------------------------------------------------
void GameMode_CCD_3D::AddVertsForCompass( std::vector<Vertex_PCU>&compassVerts, Vec3 startPosition, float axisLength, float axisThickness ) const
{
...
//----------------------------------------------------------------------------------------------------------------------
void GameMode_CCD_3D::UpdateTargetInput( float deltaSeconds )
{
...
//----------------------------------------------------------------------------------------------------------------------
void GameMode_CCD_3D::UpdateCreature( float deltaSeconds )
{
	//----------------------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------------------------
void GameMode_CCD_3D::InitializeIK_ChainCCD()
{
	//----------------------------------------------------------------------------------------------------------------------
	// Creature
	//----------------------------------------------------------------------------------------------------------------------
	...
	m_ikChain_CCD->AddNewJointToChain( Vec3( m_boneLength, 0.0f, 0.0f ), EulerAngles() );		// Child 2
   	...
   	m_ikChain_CCD->AddNewJointToChain( Vec3( m_boneLength, 0.0f, 0.0f ), EulerAngles() );		// Child 5
   	m_ikChain_CCD->AddNewJointToChain( Vec3( m_boneLength, 0.0f, 0.0f ), EulerAngles() );		// Child 6
   	m_ikChain_CCD->AddNewJointToChain( Vec3( m_boneLength, 0.0f, 0.0f ), EulerAngles() );		// Child 7
   	m_ikChain_CCD->AddNewJointToChain( Vec3( m_boneLength, 0.0f, 0.0f ), EulerAngles() );		// Child 8
	// Actual parameters
	SetIK_ChainConstraints();
	m_ikChain_CCD->m_target.m_currentPos_WS = Vec3( 80.0f, 0.0f, 0.0f );
	m_ikChain_CCD->m_numIterations		    = 100;
...
//	m_ikChain_CCD->m_target.m_currentPos_WS = Vec3( 10.0f, 50.0f, -15.0f );
...
//	m_ikChain_CCD->m_eulerAngles_WS			= EulerAngles( 0.0f, 0.0f, 0.0f );
	...
	//----------------------------------------------------------------------------------------------------------------------
	// Init targetMesh (sword/staff)
	//----------------------------------------------------------------------------------------------------------------------
	// Sword blade/handle
// 	Vec3 start	= Vec3::ZERO + ( m_ikChain_CCD->m_target.m_leftDir * 5.0f );
// 	Vec3 end	= Vec3::ZERO - ( m_ikChain_CCD->m_target.m_leftDir * 20.0f );
// 	AddVertsForCylinder3D( m_ikChain_CCD->m_target.m_meshList, start, end, 0.5f );
// 	// Sword guard 
// 	start	= Vec3::ZERO - ( m_ikChain_CCD->m_target.m_leftDir * 2.0f ) + ( m_ikChain_CCD->m_target.m_upDir * 3.0f );
// 	end		= Vec3::ZERO - ( m_ikChain_CCD->m_target.m_leftDir * 2.0f ) - ( m_ikChain_CCD->m_target.m_upDir * 3.0f );
//	AddVertsForCylinder3D( m_ikChain_CCD->m_target.m_meshList, start, end, 0.5f );
...
//----------------------------------------------------------------------------------------------------------------------
void GameMode_CCD_3D::SetIK_ChainConstraints()
{
...
//			currentJoint->SetConstraints_YPR( FloatRange( -75.0f, 75.0f ), FloatRange( -75.0f, 75.0f ), FloatRange( -25.0f, 25.0f ) );
//			currentJoint->SetConstraints_YPR( FloatRange( -75.0f, 75.0f ), FloatRange( -89.9f, 89.9f ), FloatRange( -1.0f, 1.0f ) );
		}
	}
}
