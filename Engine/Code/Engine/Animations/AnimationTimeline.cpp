...
//----------------------------------------------------------------------------------------------------------------------
void AnimationTimeline::RenderAnimPose( std::vector<Vertex_PCU>& verts, float length, float thickness ) const
{
	if ( m_isVisible )
	{
		m_animPoseToRender.AddVertsForDebugJointsAndBones( verts );
		m_animPoseToRender.DebugDrawJointBasis( verts, m_jointID, length, thickness );
	}
}
...
// 		} 
		...
		AddVertsForAABB2D( verts, m_infoBounds_rotationQuat, Rgba8::DARK_BLUE );		
		// Rotation info background
		AddVertsForAABB2D( verts, m_infoBounds_rotationEuler, Rgba8::DARK_BLUE );		
		// Scale info background
		AddVertsForAABB2D( verts, m_infoBounds_scale, Rgba8::DARK_BLUE );		

		// Animation Clip Name
		AnimationClip* animClipToPlay   = m_animClipList[m_curAnimClipIndex];
		std::string animClipName		= "ClipName: " + animClipToPlay->GetName();
		...
		std::string rotationString		= Stringf( "Joint Rotation Quat:  X:%0.2f, Y:%0.2f, Z:%0.2f, W:%0.2f", jointRot.x, jointRot.y, jointRot.z, jointRot.w );
		bitmapFont->AddVertsForTextInBox2D( textVerts, m_infoBounds_rotationQuat, m_textHeight, rotationString, Rgba8::WHITE, 1.0f, alignment );	
		// Rotation info
 		EulerAngles const& jointEuler	= EulerAngles::GetAsEuler_XFwd_YLeft_ZUp( jointRot );
 		rotationString					= Stringf( "Joint Rotation Euler: Y:%0.2f, P:%0.2f, R:%0.2f", jointEuler.m_yawDegrees, jointEuler.m_pitchDegrees, jointEuler.m_rollDegrees );
 		bitmapFont->AddVertsForTextInBox2D( textVerts, m_infoBounds_rotationEuler, m_textHeight, rotationString, Rgba8::WHITE, 1.0f, alignment );	
		// Scale info
...
void AnimationTimeline::UpdateCaretPos( Vec2 const& cursorPosNormalized, float SCREEN_SIZE_X )
{
	...
	AnimationClip* animClipToPlay	= m_animClipList[ m_curAnimClipIndex ];
 	float range						= animClipToPlay->m_endTime - animClipToPlay->m_startTime;
 	m_caretTime						= animClipToPlay->m_startTime + ( range * fractionWithinRange );
...
{
	AnimationClip* animClipToPlay	= m_animClipList[ m_curAnimClipIndex ];
	AnimationPose* animPoseToPlay	= m_animPoseList[ m_curAnimClipIndex ];
	float sampleTime				= RangeMapClamped( m_caretPos.x, m_bounds.m_mins.x, m_bounds.m_maxs.x, animClipToPlay->m_startTime, animClipToPlay->m_endTime );
	m_animPoseToRender				= animClipToPlay->SampleAnimPoseAtTime( sampleTime, *animPoseToPlay );
...
//----------------------------------------------------------------------------------------------------------------------
void AnimationTimeline::IncrementJointIndex()
{
	m_jointID++;
	// "Out of bounds" check
	if ( m_jointID >= m_animPoseToRender.m_jointList.size() )
	{
		m_jointID = 0;
	}
}
...
//----------------------------------------------------------------------------------------------------------------------
void AnimationTimeline::DecrementJointIndex()
{
	m_jointID--;
	// "Out of bounds" check
	if ( m_jointID < 0 )
	{
		m_jointID = ( int( m_animPoseToRender.m_jointList.size() ) - 1 );
	}
}
...
void AnimationTimeline::UpdateKeyframePositions( int numSlices, float keyframeRadius )
{
	...
	// AddVertsForDisc at rangeMappedPos
	AnimationClip* animClipToPlay	= m_animClipList[ m_curAnimClipIndex ];
	float animClipStartTime			= animClipToPlay->m_startTime;
	float animClipEndTime			= animClipToPlay->m_endTime;
	float boundsMinX				= m_bounds.m_mins.x;
	float boundsMaxX				= m_bounds.m_maxs.x;
	float boundsMaxY				= m_bounds.m_maxs.y;
	for ( int i = 0; i < animClipToPlay->m_jointCurveList.size(); i++ )
	{
		JointAnimationCurve	const& curJointCurve	 = animClipToPlay->m_jointCurveList[i]; 
...
void AnimationTimeline::UpdateInfoBounds()
{
	...
	Vec2 newMins;
	float offset				= 12.0f;		// #ToDo: Make 'offset' a parameter instead of a hard coded number
	TransformPosition2D( newMins, Vec2::X_FWD, Vec2::Y_LEFT, Vec2(m_bounds.m_mins.x, m_bounds.m_maxs.y + offset) );
	...
	// Transform position for joint rotation (Quats) info bounds
	newMins						= Vec2 ( newMins.x, newMins.y - m_textHeight );
	newMaxs						= Vec2 ( m_infoBoundsMaxX, newMins.y + m_infoBoundsMaxY );
	m_infoBounds_rotationQuat	= AABB2( newMins, newMaxs );

	// Transform position for joint rotation (Euler) info bounds
	newMins						= Vec2 ( newMins.x, newMins.y - m_textHeight );
	newMaxs						= Vec2 ( m_infoBoundsMaxX, newMins.y + m_infoBoundsMaxY );
	m_infoBounds_rotationEuler	= AABB2( newMins, newMaxs );

	// Transform position for joint scale info bounds
	newMins						= Vec2 ( newMins.x, newMins.y - m_textHeight );
	newMaxs						= Vec2 ( m_infoBoundsMaxX, newMins.y + m_infoBoundsMaxY );
...
void AnimationTimeline::ClampInfoBounds()
{
	...
	// Clamp m_infoBounds_rotation (quats) maxes
	width								= m_infoBounds_rotationQuat.m_maxs.x - m_infoBounds_rotationQuat.m_mins.x;
	m_infoBounds_rotationQuat.m_maxs.x	= GetClamped( m_infoBounds_rotationQuat.m_maxs.x, 0.0f, m_infoBounds_rotationQuat.m_maxs.x );
	m_infoBounds_rotationQuat.m_mins.x	= m_infoBounds_rotationQuat.m_maxs.x - width;

	// Clamp m_infoBounds_rotation (Euler) maxes
	width								= m_infoBounds_rotationEuler.m_maxs.x - m_infoBounds_rotationEuler.m_mins.x;
	m_infoBounds_rotationEuler.m_maxs.x	= GetClamped( m_infoBounds_rotationEuler.m_maxs.x, 0.0f, m_infoBounds_rotationEuler.m_maxs.x );
	m_infoBounds_rotationEuler.m_mins.x	= m_infoBounds_rotationEuler.m_maxs.x - width;
