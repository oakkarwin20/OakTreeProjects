...
public:
	...
	void RenderAnimPose( std::vector<Vertex_PCU>& verts, float length, float thickness ) const;
...
public:
	...
	AABB2			m_infoBounds_rotationQuat	= AABB2( Vec2( m_bounds.m_mins.x, m_bounds.m_mins.y ), Vec2( m_bounds.m_maxs.x * 0.3f, m_bounds.m_maxs.y * 0.3f ) ); 
	AABB2			m_infoBounds_rotationEuler	= AABB2( Vec2( m_bounds.m_mins.x, m_bounds.m_mins.y ), Vec2( m_bounds.m_maxs.x * 0.3f, m_bounds.m_maxs.y * 0.3f ) ); 
...
private:
	...
	// #ToDo: add variables to init for setting disc slices and colors
};
