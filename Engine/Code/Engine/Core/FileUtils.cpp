...
//----------------------------------------------------------------------------------------------------------------------
void WriteBinaryBufferToFile( std::vector<unsigned char>& outBuffer, const std::string& fileName )
{
	std::ofstream outputFile( fileName, std::ios::binary | std::ios::out );
	outputFile.write( reinterpret_cast<char*>( outBuffer.data() ), outBuffer.size() );
	outputFile.close();
}
