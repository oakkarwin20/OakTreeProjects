...
{
	Vec3 worldUp	  = Vec3::Z_UP;
	Vec3 worldleft	  = Vec3::Y_LEFT;

	Vec3 displacement = endPos - startPos;
	Vec3 iBasis		  = displacement.GetNormalized();
	Vec3 jBasis		  = CrossProduct3D( worldUp, iBasis );
	float jMagnitude  = jBasis.GetLength();
	if ( CompareIfFloatsAreEqual( jMagnitude, 0.0f, 0.01f ) )
	{
		// #ToDo: check for crossProduct zero
		jBasis = worldleft;
	}
	jBasis.Normalize();
	...
	//----------------------------------------------------------------------------------------------------------------------
	// Ground BL, BR, TL, TR
	Vec3 a =  iBasis * thickness;
	Vec3 b =  jBasis * thickness;
	Vec3 c =  kBasis * thickness;
	Vec3 d = a + b + c;
	Vec3 GBL = mins + (iBasis * -1.0f) * thickness + (jBasis *  1.0f) * thickness + (kBasis *  1.0f) * thickness;
	Vec3 GBR = mins + (iBasis * -1.0f) * thickness + (jBasis * -1.0f) * thickness + (kBasis *  1.0f) * thickness;
	Vec3 GTL = mins + (iBasis * -1.0f) * thickness + (jBasis *  1.0f) * thickness + (kBasis * -1.0f) * thickness;
	Vec3 GTR = mins + (iBasis * -1.0f) * thickness + (jBasis * -1.0f) * thickness + (kBasis * -1.0f) * thickness;
	// Sky BL, BR, TL, TR
	Vec3 SBL = maxs + (iBasis *  1.0f) * thickness + (jBasis *  1.0f) * thickness + (kBasis  *  1.0f) * thickness;
	Vec3 SBR = maxs + (iBasis *  1.0f) * thickness + (jBasis * -1.0f) * thickness + (kBasis  *  1.0f) * thickness;
	Vec3 STL = maxs + (iBasis *  1.0f) * thickness + (jBasis *  1.0f) * thickness + (kBasis  * -1.0f) * thickness;
	Vec3 STR = maxs + (iBasis *  1.0f) * thickness + (jBasis * -1.0f) * thickness + (kBasis  * -1.0f) * thickness;

	// West Face
	AddVertsForQuad3D( verts, GBL, GBR, SBR, SBL, color );
	// East Face						 
	AddVertsForQuad3D( verts, GTR, GTL, STL, STR, color );
	// North Face						    	
	AddVertsForQuad3D( verts, GTL, GBL, SBL, STL, color );
	// South Face						    	
	AddVertsForQuad3D( verts, GBR, GTR, STR, SBR, color );
	// Sky Face							    	
	AddVertsForQuad3D( verts, SBL, SBR, STR, STL, color );
	// Ground Face						    	
	AddVertsForQuad3D( verts, GTL, GTR, GBR, GBL, color );

	//----------------------------------------------------------------------------------------------------------------------
	// Debug draws
	// IJK basis
//	AddVertsForArrow3D( verts, startPos, iBasis * 20.0f, 0.5f, Rgba8::RED   );
//	AddVertsForArrow3D( verts, startPos, jBasis * 20.0f, 0.5f, Rgba8::GREEN );
//	AddVertsForArrow3D( verts, startPos, kBasis * 20.0f, 0.5f, Rgba8::BLUE  );
//	// vert positions
//	AddVertsForSphere3D( verts, GBL, 1.0f, 16.0f, 16.0f, Rgba8::RED );
//	AddVertsForSphere3D( verts, GBR, 1.0f, 16.0f, 16.0f, Rgba8::GREEN );
//	AddVertsForSphere3D( verts, GTL, 1.0f, 16.0f, 16.0f, Rgba8::BLUE );
//	AddVertsForSphere3D( verts, GTR, 1.0f, 16.0f, 16.0f, Rgba8::MAGENTA );
//	AddVertsForSphere3D( verts, SBL, 1.0f, 16.0f, 16.0f, Rgba8::DARK_RED );
//	AddVertsForSphere3D( verts, SBR, 1.0f, 16.0f, 16.0f, Rgba8::DARK_GREEN );
//	AddVertsForSphere3D( verts, STL, 1.0f, 16.0f, 16.0f, Rgba8::DARK_BLUE );
//	AddVertsForSphere3D( verts, STR, 1.0f, 16.0f, 16.0f, Rgba8::CYAN );
...
//----------------------------------------------------------------------------------------------------------------------
void AddVertsForDisc2D( std::vector<Vertex_PCU>& verts, Vec2 centerPos, float radius, Rgba8 color, int numPizzaSlices )
{
	float degreesPerSlice = 360.0f / (float)numPizzaSlices;
	for ( int i = 0; i < numPizzaSlices; i++ )
	{
...
//----------------------------------------------------------------------------------------------------------------------
void AddVertsForConvexPoly2D( std::vector<Vertex_PCU>& verts, ConvexPoly2D const& convexPoly2D, Rgba8 const& color )
{
	std::vector<Vec2> const& ccwOrderedPoints = convexPoly2D.GetCcwOrderedPoints();
	// Only loop till (size - 2) since thats the last tri we will draw for this convexPoly
	for ( int i = 0; i < (ccwOrderedPoints.size() - 2); i++ )
	{
		Vec2 pointA = ccwOrderedPoints[0	];
		Vec2 pointB = ccwOrderedPoints[i + 1];
		Vec2 pointC = ccwOrderedPoints[i + 2];

		verts.push_back( Vertex_PCU( Vec3( pointA.x, pointA.y, 0.0f ), color, Vec2( 0.0f, 0.0f ) ) );
		verts.push_back( Vertex_PCU( Vec3( pointB.x, pointB.y, 0.0f ), color, Vec2( 1.0f, 0.0f ) ) );
		verts.push_back( Vertex_PCU( Vec3( pointC.x, pointC.y, 0.0f ), color, Vec2( 0.0f, 1.0f ) ) );
	}
}


//----------------------------------------------------------------------------------------------------------------------
void AddVertsForConvexPoly2D_Border( std::vector<Vertex_PCU>& verts, ConvexPoly2D const& convexPoly2D, Rgba8 const& color, float thickness )
{
	std::vector<Vec2> const& ccwOrderedPoints = convexPoly2D.GetCcwOrderedPoints();
	// Only loop till (size - 1) since thats the last line segment border to draw for this convexPoly
	int maxIter = int( ccwOrderedPoints.size() );
	for ( int i = 0; i < maxIter; i++ )
	{
		Vec2 pointA = ccwOrderedPoints[i + 0];
		Vec2 pointB = Vec2::ZERO;
		if ( i == (maxIter - 1) )
		{
			pointB = ccwOrderedPoints[0];
		}
		else
		{
			pointB = ccwOrderedPoints[ i + 1 ];
		}		
		AddVertsForLineSegment2D( verts, pointA, pointB, thickness, color );
	}
}


//----------------------------------------------------------------------------------------------------------------------
void AddVertsForConvexHull2D_Planes( std::vector<Vertex_PCU>& verts, ConvexHull2D const& convexHull2D, float lineLength, Rgba8 const& color, float planeThickness, bool renderDebugNormals, float debugNormalLength, float arrowSize, Rgba8 debugNormalColor )
{
	std::vector<Plane2D> planesList = convexHull2D.GetPlanesList();
	for ( int i = 0; i < planesList.size(); i++ )
	{
		// Render planes
		Plane2D& curPlane				 = planesList[i];
		Vec2 planeLeftDir				 = -curPlane.m_normal.GetRotated90Degrees();
		Vec2 planeRightDir				 =  curPlane.m_normal.GetRotated90Degrees();
		Vec2 nearestPointToOriginOnPlane = curPlane.GetNearestPointToOrigin();
		Vec2  lineEndLeft = nearestPointToOriginOnPlane + ( planeLeftDir  * lineLength );
		Vec2 lineEndRight = nearestPointToOriginOnPlane + ( planeRightDir * lineLength );
		AddVertsForLineSegment2D( verts, nearestPointToOriginOnPlane,  lineEndLeft, planeThickness, color );
		AddVertsForLineSegment2D( verts, nearestPointToOriginOnPlane, lineEndRight, planeThickness, color );
		// Render plane normal
		if ( renderDebugNormals )
		{
			Vec2 debugNormalStart = curPlane.m_debugNormalPos;
			Vec2 debugNormalEnd	  = debugNormalStart + ( curPlane.m_normal * debugNormalLength );
			AddVertsForArrow2D( verts, debugNormalStart, debugNormalEnd, arrowSize, planeThickness, debugNormalColor );
		}
	}
}
...
//----------------------------------------------------------------------------------------------------------------------
void AddVertsForDisc3D_XY( std::vector<Vertex_PCU>& verts, Vec3 const& discCenter, float discRadius, Rgba8 const& color )
{
	int		numPizzaSlices		= 48;
	float	degreesPerSlice		= 360.0f / (float)numPizzaSlices;
	for ( int i = 0; i < numPizzaSlices; i++ )
	{
		...
		Vec2 disp1 = Vec2::MakeFromPolarDegrees( startOrientation, discRadius );
		Vec2 disp2 = Vec2::MakeFromPolarDegrees(   endOrientation, discRadius );

		Vec2 discCenter2D	= Vec2( discCenter.x, discCenter.y );
		Vec2 pos0			= discCenter2D;
		Vec2 pos1			= discCenter2D + disp1;
		Vec2 pos2			= discCenter2D + disp2;
		// Pushback
		verts.push_back( Vertex_PCU( Vec3( pos0.x, pos0.y, discCenter.z ), color, Vec2( 0.0f, 1.0f ) ) );
		verts.push_back( Vertex_PCU( Vec3( pos1.x, pos1.y, discCenter.z ), color, Vec2( 0.0f, 1.0f ) ) );
		verts.push_back( Vertex_PCU( Vec3( pos2.x, pos2.y, discCenter.z ), color, Vec2( 0.0f, 1.0f ) ) );
	}
}
...
//----------------------------------------------------------------------------------------------------------------------
void AddVertsForCapsule3D( std::vector<Vertex_PCU>& verts, Vec3 const& start, Vec3 const& end, float startRadius, float endRadius, Rgba8 const& color, AABB2 const& UVs, float numSlices, float numStacks )
{
	AddVertsForCylinder3D_WallsOnly( verts, start, end, startRadius, endRadius, Vec2::ZERO, Vec2::ZERO, color, UVs, numSlices );		// Walls
	AddVertsForHemisphere3D( verts, start,  end, endRadius,   Vec2::ZERO, numSlices, numStacks, color, UVs );							// Top hemisphere
	AddVertsForHemisphere3D( verts, end,  start, startRadius, Vec2::ZERO, numSlices, numStacks, color, UVs );							// Bottom hemisphere
}


//----------------------------------------------------------------------------------------------------------------------
void AddVertsForCapsule3D_TaperedEllipse( std::vector<Vertex_PCU>& verts, Vec3 const& start, Vec3 const& end, float startRadius, float endRadius, Vec2 const& startRadiusScalarXY, Vec2 const& endRadiusScalarXY, Rgba8 const& color, AABB2 const& UVs, float numSlices, float numStacks )
{
	AddVertsForCylinder3D_WallsOnly( verts, start, end, startRadius, endRadius, startRadiusScalarXY, endRadiusScalarXY, color, UVs, numSlices );		// Walls
	AddVertsForHemisphere3D( verts, start,  end, endRadius, endRadiusScalarXY, numSlices, numStacks, color, UVs );		// Top, end hemisphere
	AddVertsForHemisphere3D( verts, end,  start, startRadius, startRadiusScalarXY, numSlices, numStacks, color, UVs );	// Bottom, start hemisphere
}
...
//----------------------------------------------------------------------------------------------------------------------
void AddVertsForCylinder3D( std::vector<Vertex_PCU>& verts, Vec3 const& start, Vec3 const& end, float radius, Rgba8 const& color, AABB2 const& UVs, float numSlices )
{
	//----------------------------------------------------------------------------------------------------------------------
	// Create zCylinder
	...
	if ( CompareIfFloatsAreEqual( jBasisLength, 0.0f, 0.001f ) )
	{
...
//----------------------------------------------------------------------------------------------------------------------
void AddVertsForCylinder3D_WallsOnly( std::vector<Vertex_PCU>& verts, Vec3 const& start, Vec3 const& end, float startRadius, float endRadius, Vec2 const& startRadiusScalarXY, Vec2 const& endRadiusScalarXY, Rgba8 const& color, AABB2 const& UVs, float numSlices )
{
	//----------------------------------------------------------------------------------------------------------------------
	// Create zCylinder
	std::vector<Vertex_PCU> localVerts;
	AddVertsForCylinderZ3D_WallsOnly(localVerts, start, end, startRadius, endRadius, (float)numSlices, startRadiusScalarXY, endRadiusScalarXY, color, UVs );
...
void AddVertsForCylinder3D_WallsOnly( std::vector<Vertex_PCUTBN>& verts, Vec3 const& start, Vec3 const& end, float startRadius, float endRadius, Vec2 const& startRadiusScalarXY /*= Vec2::ZERO*/, Vec2 const& endRadiusScalarXY /*= Vec2::ZERO*/, Rgba8 const& color /*= Rgba8::WHITE*/, AABB2 const& UVs /*= AABB2::ZERO_TO_ONE*/, float numSlices /*= 16.0f */ )
{
	...
	AddVertsForCylinderZ3D_WallsOnly( localVerts, start, end, startRadius, endRadius, numSlices, startRadiusScalarXY, endRadiusScalarXY, color, UVs );
...
//----------------------------------------------------------------------------------------------------------------------
void AddVertsForCylinderZ3D_WallsOnly( std::vector<Vertex_PCU>& verts, Vec3 const& start, Vec3 const& end, float startRadius, float endRadius, float numSlices, Vec2 const& startRadiusScalarXY, Vec2 const& endRadiusScalarXY, Rgba8 const& tint, AABB2 const& UVs )
{
	Vec2 centerXY				= Vec2::ZERO;
	Vec3 dispSE					= end - start;	
	FloatRange cylinderZRange	= FloatRange(0.0f, dispSE.GetLength());
	float yawDegreesPerSlice = ( 360.0f / numSlices );
	for ( int currentSlice = 0; currentSlice < numSlices; currentSlice++ )
	{
		// Compute current and next yawDegrees for cylinder
		float currentYawDegrees = static_cast<float>( currentSlice ) * yawDegreesPerSlice;
		float nextYawDegrees	= currentYawDegrees + yawDegreesPerSlice;
		// Make positions from current and next yawDegrees
		Vec2 currentPointBottom	= Vec2::ZERO + Vec2::MakeFromPolarDegrees_Skewed(  currentYawDegrees,	startRadius, startRadiusScalarXY );
		Vec2 nextPointBottom	= Vec2::ZERO + Vec2::MakeFromPolarDegrees_Skewed(		nextYawDegrees,	startRadius, startRadiusScalarXY );
		Vec2 currentPointTop	= Vec2::ZERO + Vec2::MakeFromPolarDegrees_Skewed(  currentYawDegrees,	endRadius,	 endRadiusScalarXY   );
		Vec2 nextPointTop		= Vec2::ZERO + Vec2::MakeFromPolarDegrees_Skewed(		nextYawDegrees,	endRadius,	 endRadiusScalarXY   );
		// Compute Quad vert positions (walls)
		Vec3 bottomLeftMinZ		= Vec3(	 currentPointBottom.x,  currentPointBottom.y, cylinderZRange.m_min );		
		Vec3 bottomRightMinZ	= Vec3(		nextPointBottom.x,	   nextPointBottom.y, cylinderZRange.m_min );	
		Vec3 topLeftMaxZ		= Vec3(		currentPointTop.x,     currentPointTop.y, cylinderZRange.m_max );	
		Vec3 topRightMaxZ		= Vec3(		   nextPointTop.x,	      nextPointTop.y, cylinderZRange.m_max );		
...
void AddVertsForCylinderZ3D_WallsOnly( std::vector<Vertex_PCUTBN>& verts, Vec3 const& start, Vec3 const& end, float startRadius, float endRadius, float numSlices, Vec2 const& startRadiusScalarXY /*= Vec2::ZERO*/, Vec2 const& endRadiusScalarXY /*= Vec2::ZERO*/, Rgba8 const& tint /*= Rgba8::WHITE*/, AABB2 const& UVs /*= AABB2::ZERO_TO_ONE */ )
{
	Vec2  centerXY				= Vec2::ZERO;
	Vec3  dispSE				= end - start;	
	float lengthSE				= dispSE.GetLength();
	Vec3  end_zUp				= Vec3::ZERO + ( Vec3::Z_UP * lengthSE );
	FloatRange cylinderZRange	= FloatRange(0.0f, lengthSE );
	float yawDegreesPerSlice = ( 360.0f / numSlices );
	for ( int currentSlice = 0; currentSlice < numSlices; currentSlice++ )
	{
		// Compute current and next yawDegrees for cylinder
		...
		// Fix normals 
		//----------------------------------------------------------------------------------------------------------------------
		// center to vert positions
		// get last 6 verts, then compute and update normals
		int vertsSize = int( verts.size() - 1 );
		Vertex_PCUTBN& BL	= verts[ vertsSize - 5 ];
		Vertex_PCUTBN& BR	= verts[ vertsSize - 4 ];
		Vertex_PCUTBN& TR	= verts[ vertsSize - 3 ];
		Vertex_PCUTBN& BL_2	= verts[ vertsSize - 2 ];
		Vertex_PCUTBN& TR_2	= verts[ vertsSize - 1 ];
		Vertex_PCUTBN& TL	= verts[ vertsSize - 0 ];

		BL.m_normal		= ( BL	.m_position - Vec3::ZERO ).GetNormalized();		
		BL_2.m_normal	= ( BL_2.m_position - Vec3::ZERO ).GetNormalized();		
		BR.m_normal		= ( BR	.m_position - Vec3::ZERO ).GetNormalized();		
		TR.m_normal		= ( TR	.m_position - end_zUp	 ).GetNormalized();		
		TR_2.m_normal	= ( TR_2.m_position - end_zUp	 ).GetNormalized();		
		TL.m_normal		= ( TL	.m_position - end_zUp	 ).GetNormalized();		
	}
}


//----------------------------------------------------------------------------------------------------------------------
void AddVertsForCylinder3D_Tapered( std::vector<Vertex_PCU>& verts, Vec3 const& start, Vec3 const& end, float startRadius, float endRadius, Rgba8 const& color, AABB2 const& UVs, int numSlices )
{
	//----------------------------------------------------------------------------------------------------------------------
	// Create zCylinder
	std::vector<Vertex_PCU> localVerts;
	...
	Vec2 centerXY		= Vec2(0.f, 0.f);
	FloatRange minMaxZ	= FloatRange(0.0f, dispSE.GetLength());
	AddVertsForCylinderZ3D_Tapered( localVerts, start, end, startRadius, endRadius, color, UVs, numSlices );
...
//----------------------------------------------------------------------------------------------------------------------
void AddVertsForCylinderZ3D_Tapered( std::vector<Vertex_PCU>& verts, Vec3 const& start, Vec3 const& end, float startRadius, float endRadius, Rgba8 const& color, AABB2 const& UVs, int numSlices )
{
	Vec2 centerXY				= Vec2::ZERO;
	Vec3 dispSE					= end - start;	
	FloatRange cylinderZRange	= FloatRange(0.0f, dispSE.GetLength());
	...
	for ( int currentSlice = 0; currentSlice < numSlices; currentSlice++ )
	{
		...
		float nextYawDegrees	= currentYawDegrees + yawDegreesPerSlice;
		// Make positions from current and next yawDegrees
		Vec2 currentPointBottom	= centerXY + Vec2::MakeFromPolarDegrees( currentYawDegrees,	startRadius );
		Vec2 nextPointBottom	= centerXY + Vec2::MakeFromPolarDegrees(	nextYawDegrees,	startRadius );
		Vec2 currentPointTop	= centerXY + Vec2::MakeFromPolarDegrees( currentYawDegrees,	endRadius   );
		Vec2 nextPointTop		= centerXY + Vec2::MakeFromPolarDegrees(	nextYawDegrees,	endRadius   );
		// Compute Quad vert positions (walls)
		Vec3 bottomLeftMinZ		= Vec3(	 currentPointBottom.x,  currentPointBottom.y, cylinderZRange.m_min );		
		Vec3 bottomRightMinZ	= Vec3(		nextPointBottom.x,	   nextPointBottom.y, cylinderZRange.m_min );	
		Vec3 topLeftMaxZ		= Vec3(		currentPointTop.x,     currentPointTop.y, cylinderZRange.m_max );	
		...
		AddVertsForQuad3D( verts, bottomLeftMinZ, bottomRightMinZ, topRightMaxZ, topLeftMaxZ, color, AABB2(UV_bottomLeft, UV_topRight) );
		...
		// Pushback verts
		// Top face 
		verts.push_back( Vertex_PCU(	 Vec3(centerXY.x, centerXY.y, cylinderZRange.m_max), color, UV_center			  ) );
		verts.push_back( Vertex_PCU(										    topLeftMaxZ, color, UV_currentOrientation ) );
		verts.push_back( Vertex_PCU(										   topRightMaxZ, color, UV_nextOrientation	  ) );

		// Bottom face 
		verts.push_back( Vertex_PCU(	 Vec3(centerXY.x, centerXY.y, cylinderZRange.m_min), color, UV_center					 ) );
		verts.push_back( Vertex_PCU(										bottomRightMinZ, color, UV_nextOrientationFlipped	 ) );
		verts.push_back( Vertex_PCU(										 bottomLeftMinZ, color, UV_currentOrientationFlipped ) );
	}
}
...
//----------------------------------------------------------------------------------------------------------------------
void AddVertsForEllipseZ3D_Tapered( std::vector<Vertex_PCU>& verts, Vec3 const& start, Vec3 const& end, float startRadius, float endRadius, Vec2 startRadiusScalarXY, Vec2 endRadiusScalarXY, Rgba8 const& color, AABB2 const& UVs, int numSlices )
{
	Vec2 centerXY				= Vec2::ZERO;
	Vec3 dispSE					= end - start;	
	FloatRange cylinderZRange	= FloatRange(0.0f, dispSE.GetLength());
	...
	for ( int currentSlice = 0; currentSlice < numSlices; currentSlice++ )
	{
		...
		float nextYawDegrees	= currentYawDegrees + yawDegreesPerSlice;
		// Make positions from current and next yawDegrees
		Vec2 currentPointBottom	= centerXY + Vec2::MakeFromPolarDegrees_Skewed(  currentYawDegrees,	startRadius, startRadiusScalarXY );
		Vec2 nextPointBottom	= centerXY + Vec2::MakeFromPolarDegrees_Skewed(		nextYawDegrees,	startRadius, startRadiusScalarXY );
		Vec2 currentPointTop	= centerXY + Vec2::MakeFromPolarDegrees_Skewed(  currentYawDegrees,	endRadius,	 endRadiusScalarXY   );
		Vec2 nextPointTop		= centerXY + Vec2::MakeFromPolarDegrees_Skewed(		nextYawDegrees,	endRadius,	 endRadiusScalarXY   );
		// Compute Quad vert positions (walls)
		Vec3 bottomLeftMinZ		= Vec3(	 currentPointBottom.x,  currentPointBottom.y, cylinderZRange.m_min );		
		Vec3 bottomRightMinZ	= Vec3(		nextPointBottom.x,	   nextPointBottom.y, cylinderZRange.m_min );	
		Vec3 topLeftMaxZ		= Vec3(		currentPointTop.x,     currentPointTop.y, cylinderZRange.m_max );	
...
//----------------------------------------------------------------------------------------------------------------------
void AddVertsForEllipse3D_Tapered( std::vector<Vertex_PCU>& verts, Vec3 const& start, Vec3 const& end, float startRadius, float endRadius, Vec2 startRadiusScalarXY, Vec2 endRadiusScalarXY, Rgba8 const& color, AABB2 const& UVs, int numSlices )
{
	//----------------------------------------------------------------------------------------------------------------------
	// Create zEllipse
	std::vector<Vertex_PCU> localVerts;
	...
	Vec2 centerXY		= Vec2(0.f, 0.f);
	FloatRange minMaxZ	= FloatRange(0.0f, dispSE.GetLength());
	AddVertsForEllipseZ3D_Tapered( localVerts, start, end, startRadius, endRadius, startRadiusScalarXY, endRadiusScalarXY, color, UVs, numSlices );
	...
	// Render basis
	AddVertsForArrow3D( localVerts, Vec3::ZERO, iBasis * 10.0f, 0.5f, Rgba8::RED   );
	AddVertsForArrow3D( localVerts, Vec3::ZERO, jBasis * 10.0f, 0.5f, Rgba8::GREEN );
	AddVertsForArrow3D( localVerts, Vec3::ZERO, kBasis * 10.0f, 0.5f, Rgba8::BLUE  );
...
//----------------------------------------------------------------------------------------------------------------------
void AddVertsForHemisphere3D( std::vector<Vertex_PCU>& verts, Vec3 const& start, Vec3 const& end, float radius, Vec2 const& radiusScalarXY, float numSlices, float numStacks, Rgba8 const& color, AABB2 const& UVs )
{
	//----------------------------------------------------------------------------------------------------------------------
	// Create a Z+ hemisphere by default
	//----------------------------------------------------------------------------------------------------------------------
	std::vector<Vertex_PCU> localVerts;
	AddVertsForHemisphereZ3D( localVerts, Vec3::ZERO, radius, radiusScalarXY, numSlices, numStacks, color, UVs );

	//----------------------------------------------------------------------------------------------------------------------
	// Transform verts into desired basis
	//----------------------------------------------------------------------------------------------------------------------
	// Calculate I,J,K basis for zCylinder
	Mat44 matrix;
	Vec3 zSkyward	= Vec3( 0.0f, 0.0f, 1.0f );
...
//----------------------------------------------------------------------------------------------------------------------
void AddVertsForHemisphereZ3D( std::vector<Vertex_PCU>& verts, Vec3 const& center, float radius, Vec2 const& radiusScalarXY, float numSlices, float numStacks, Rgba8 const& color, AABB2 const& UVs )
{
	float yawDegreesPerSlice	= ( 360.0f / numSlices );
	float pitchDegreesPerStack	= (  90.0f / numStacks );
	for ( int currentSlice = 0; currentSlice < numSlices; currentSlice++ )
	{
		float currentYawDegrees = static_cast<float>(currentSlice) * yawDegreesPerSlice;
		...
		for ( int currentStack = 0; currentStack < numStacks; currentStack++ )		
		{
			...
			vertsTopLeft	 = center + Vec3::MakeFromSphericalDegrees(						 currentYawDegrees,						   currentPitchDegrees, radius, radiusScalarXY );		// add verts for current as TopLeft										// Current
			vertsTopRight	 = center + Vec3::MakeFromSphericalDegrees( currentYawDegrees + yawDegreesPerSlice,						   currentPitchDegrees, radius, radiusScalarXY );		// then add longitudeOffset to current to add verts for TopRight		// Yaw to the right
			vertsBottomLeft	 = center + Vec3::MakeFromSphericalDegrees(	                     currentYawDegrees, currentPitchDegrees + pitchDegreesPerStack, radius, radiusScalarXY );		// then add latitudeOffset to TopRight to add verts for BottomRight		// Yaw to the right then Pitch down (positive)
			vertsBottomRight = center + Vec3::MakeFromSphericalDegrees( currentYawDegrees + yawDegreesPerSlice, currentPitchDegrees + pitchDegreesPerStack, radius, radiusScalarXY );		// then add latitudeOffset to TopLeft  to add verts for BottomLeft		// Pitch down only from Current
...
void AddVertsForHemisphereZ3D( std::vector<Vertex_PCUTBN>& verts, Vec3 const& center, float radius, Vec2 const& radiusScalarXY /*= Vec2::ZERO*/, float numSlices /*= 16.0f*/, float numStacks /*= 16.0f*/, Rgba8 const& color /*= Rgba8::WHITE*/, AABB2 const& UVs /*= AABB2::ZERO_TO_ONE */ )
{
	...
	for ( int currentSlice = 0; currentSlice < numSlices; currentSlice++ )
	{
		...
		for ( int currentStack = 0; currentStack < numStacks; currentStack++ )		
		{
			...
			BL.m_normal		= ( BL	.m_position - center ).GetNormalized();		
			BL_2.m_normal	= ( BL_2.m_position - center ).GetNormalized();		
			BR.m_normal		= ( BR	.m_position - center ).GetNormalized();		
			TR.m_normal		= ( TR	.m_position - center ).GetNormalized();		
			TR_2.m_normal	= ( TR_2.m_position - center ).GetNormalized();		
			TL.m_normal		= ( TL	.m_position - center ).GetNormalized();
		}
	}
}
...
void AddVertsForUVSphereZWireframe3D( std::vector<Vertex_PCU>& verts, Vec3 const& center, float radius, float numSlices, float numStacks, float lineThickness, Rgba8 const& tint )
{
	...
	for ( int currentSlice = 0; currentSlice < numSlices; currentSlice++ )
	{
		...
		for ( int currentStack = 0; currentStack < numStacks; currentStack++ )		
		{
			...
			// Pushback all verts
			// Triangle 1 (BR)
			AddVertsForLineSegment3D( verts, vertsBottomLeft,  vertsBottomRight, lineThickness, tint );		// BL to BR
			AddVertsForLineSegment3D( verts, vertsBottomRight, vertsTopRight,	 lineThickness, tint );		// BR to TR
			AddVertsForLineSegment3D( verts, vertsTopRight,	   vertsBottomLeft,  lineThickness, tint );		// TR to BL
			// Debug draw points
// 			AddVertsForSphere3D( verts, vertsBottomLeft,	lineThickness + 0.5f, 3.0f, 3.0f, Rgba8::RED );
// 			AddVertsForSphere3D( verts, vertsBottomRight,	lineThickness + 0.5f, 3.0f, 3.0f, Rgba8::RED );
// 			AddVertsForSphere3D( verts, vertsTopRight,		lineThickness + 0.5f, 3.0f, 3.0f, Rgba8::RED );
// 			AddVertsForSphere3D( verts, vertsTopLeft,		lineThickness + 0.5f, 3.0f, 3.0f, Rgba8::RED );
		}
	}
}
...
void AddVertsForConeZ3D( std::vector<Vertex_PCU>& verts, Vec2 const& start, FloatRange minMaxZ, float radius, Rgba8 const& color, AABB2 const& UVs, int numSlices )
{
	...
	for ( int currentSlice = 0; currentSlice< numSlices; currentSlice++ )
	{
		...
		// V3 is Vec3
		Vec3 curPoint_V3 = Vec3( disp1V2.x, disp1V2.y, minMaxZ.m_min );
		Vec3 nextPoint_V3 = Vec3( disp2V2.x, disp2V2.y, minMaxZ.m_min );

		// Calculate UVs Disc 
		Vec2 UV_center	   = Vec2( 0.5f, 0.5f );
		Vec2 UV_discCurPoint;
		UV_discCurPoint.x  = RangeMapClamped( CosDegrees( currentYawDegrees ), -1.0f,  1.0f, UVs.m_maxs.y, UVs.m_mins.y );			// Flip input from (1, -1) to (-1, 1) since we want to wrap the texture in the reverse order
		UV_discCurPoint.y  = RangeMapClamped( SinDegrees( currentYawDegrees ),  1.0f, -1.0f, UVs.m_maxs.y, UVs.m_mins.y );
		Vec2 UV_discNextPoint;
		UV_discNextPoint.x = RangeMapClamped( CosDegrees( nextYawDegrees ),  1.0f, -1.0f, UVs.m_mins.x, UVs.m_maxs.x );
		UV_discNextPoint.y = RangeMapClamped( SinDegrees( nextYawDegrees ), -1.0f,  1.0f, UVs.m_mins.y, UVs.m_maxs.y );

		// Disc
		localVerts.push_back( Vertex_PCU( coneBaseCenter + nextPoint_V3, color, UV_discNextPoint ) );
		localVerts.push_back( Vertex_PCU( coneBaseCenter + curPoint_V3,  color, UV_discCurPoint  ) );
		localVerts.push_back( Vertex_PCU( coneBaseCenter,				 color, UV_center		 ) );

		// Flatten y values for funnel
		UV_discCurPoint.y	= 0.0f;
		UV_discNextPoint.y	= 0.0f;

		// Funnel 
		localVerts.push_back( Vertex_PCU( coneTip,						 color, UVs.m_maxs		 ) );
		localVerts.push_back( Vertex_PCU( coneBaseCenter + curPoint_V3,  color, UV_discCurPoint  ) );
		localVerts.push_back( Vertex_PCU( coneBaseCenter + nextPoint_V3, color, UV_discNextPoint ) ); 
	}
...
void AddVertsForCone3D( std::vector<Vertex_PCU>& verts, Vec3 const& start, Vec3 const& end, float radius, Rgba8 const& color, AABB2 const& UVs, int numSlices )
{
	...
	float magnitude = coneLeft.GetLength();
	if ( CompareIfFloatsAreEqual( magnitude, 0.0f, 0.001f ) )					// Set jBasis to y-axis(world left) if coneForward is parallel to z-axis(world up/sky)
	{
...
void AddVertsForArc3D_Z_Up( std::vector<Vertex_PCU>& verts, FloatRange const& floatRange, float length, Rgba8 const& color, Vec2 const& UV )
{
	...
	//----------------------------------------------------------------------------------------------------------------------
	// NEW TEST: Draw arcs min to max
	//----------------------------------------------------------------------------------------------------------------------
	float maxAngle			= floatRange.m_max;
	float minAngle			= floatRange.m_min;
 	float currentAngle		= floatRange.m_min;
 	float degreesPerSliced  = fabsf( currentAngle * 0.1f );
	if ( CompareIfFloatsAreEqual( degreesPerSliced, 0.0f, 0.01f ) )
	{
		// Ensure degrees per slice is never zero to avoid inf loop
		degreesPerSliced = 4.5f;
	}
	for ( float i = minAngle; i < maxAngle; i += degreesPerSliced )
	{
		currentAngle		= i;
	...
	//----------------------------------------------------------------------------------------------------------------------
	// (OLD APPROACH) Draw arcs min to start or Zero
	//----------------------------------------------------------------------------------------------------------------------
// 	float maxAngle		= floatRange.m_max;
// 	if ( maxAngle < 0.0f )
// 	{
...
// {
// 	int sizeVertPCU = int( vertsPCU.size() );
// 	vertsPCUTBN.reserve( sizeVertPCU );
// 	for ( int i = 0; i < sizeVertPCU; i ++ )
// 	{
// 		Vertex_PCU const& curPCU	= vertsPCU[i];
// 		Vertex_PCUTBN curTBN;
// 		curTBN.m_position			= curPCU.m_position;
// 		curTBN.m_color				= curPCU.m_color;
// 		curTBN.m_uvTexCoords		= curPCU.m_uvTexCoords;
// 		vertsPCUTBN.emplace_back( curTBN );
// 	}
// 
// 	for ( int i = 0; i < sizeVertPCU; i ++ )
// 	{
...
// 		int index2 = i + 2;
// 
// 		if ( index1 >= sizeVertPCU )
// 		{
// 			index1 -= sizeVertPCU;
// 		}
// 		if ( index2 >= sizeVertPCU )
// 		{
// 			index2 -= sizeVertPCU;
// 		}
// 
// 		//----------------------------------------------------------------------------------------------------------------------
// 		// Positions
// 		Vec3& pos_0 = vertsPCUTBN[ index0 ].m_position;
// 		Vec3& pos_1 = vertsPCUTBN[ index1 ].m_position;
// 		Vec3& pos_2 = vertsPCUTBN[ index2 ].m_position;
// 		// Normals
// 		Vec3& normal_0 = vertsPCUTBN[ index0 ].m_normal;
// 		Vec3& normal_1 = vertsPCUTBN[ index1 ].m_normal;
// 		Vec3& normal_2 = vertsPCUTBN[ index2 ].m_normal;
// 
// 		Vec3 a	 = pos_2 - pos_1;
// 		Vec3 b	 = pos_0 - pos_1;
// 		normal_0 = CrossProduct3D( b, a );
// 		normal_0.Normalize();
// 	}
// }
