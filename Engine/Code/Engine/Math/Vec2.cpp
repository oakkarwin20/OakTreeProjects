...
Vec2 Vec2::X_FWD		= Vec2(  1.0f,  0.0f );
Vec2 Vec2::Y_LEFT		= Vec2(  0.0f,  1.0f );
...
//----------------------------------------------------------------------------------------------------------------------
Vec2 const Vec2::MakeFromPolarDegrees_Skewed( float orientationDegrees, float length, Vec2 startRadiusScalarXY )
{
	float orientationRadians	= ConvertDegreesToRadians( orientationDegrees );
	Vec2 vec2InCarteisianCoords = MakeFromPolarRadians_Skewed( orientationRadians, length, startRadiusScalarXY );
	return vec2InCarteisianCoords;
}

//----------------------------------------------------------------------------------------------------------------------
Vec2 const Vec2::MakeFromPolarRadians_Skewed( float orientationRadians, float length, Vec2 startRadiusScalarXY )
{
	// Polar to Cartesian conversion formula
	// x = R * cos ( theta )
	// y = R * sine( theta )
	float cos			= cosf( orientationRadians );
	float x				= length * ( cos * startRadiusScalarXY.x );
	float sine			= sinf( orientationRadians );
	float y				= length * ( sine * startRadiusScalarXY.y );
