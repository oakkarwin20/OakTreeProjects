...
//----------------------------------------------------------------------------------------------------------------------
bool IsPointInsideConvexPoly2D( Vec2 const& testPoint, ConvexPoly2D const& convexPoly )
{
	// Loop over all indexes in convexPolyPoints
	// CrossProduct( dispCurPointToPoint, dispCurPointToNexPoint ) 
	// If ANY crossProduct is positive, point is outside
	// If ALL crossProducts are negative, point is inside
	bool isPointInside = true;
	std::vector<Vec2> orderedPoints = convexPoly.GetCcwOrderedPoints();
	for ( int i = 0; i < orderedPoints.size(); i++ )
	{
		Vec2 curPoint				 = orderedPoints[i];
		Vec2 nextPoint				 = Vec2::ZERO;
		// Handle edge case for looping out of size
		if ( i == ( orderedPoints.size() - 1 ) )
		{
			nextPoint = orderedPoints[0];
		}
		else
		{
			nextPoint = orderedPoints[i + 1];
		}
		Vec2  dispCurPointToTestPoint = testPoint - curPoint; 
		Vec2  dispCurPointToNexPoint  = nextPoint - curPoint;
		float crossResult			  = CrossProduct2D( dispCurPointToTestPoint, dispCurPointToNexPoint );
		if ( crossResult > 0.0f )
		{
			isPointInside = false;
			break;
...
//----------------------------------------------------------------------------------------------------------------------
bool IsPointInsideConvexHull2D( Vec2 const& testPoint, ConvexHull2D const& convexHull )
{
	// Loop over all planes in convexHull
	// GetSignedAltitude for testPoint to plane
	// if ANY signedAltitude for testPoint is POSITIVE, point is outside
	// if ALL signedAltitude for testPoint is NEGATIVE, point is inside
	bool isPointInside = true;
	std::vector<Plane2D> planesList = convexHull.GetPlanesList();
	for ( int i = 0; i < planesList.size(); i++ )
	{
		Plane2D& curPlane		= planesList[i];
		float signedAltitude	= curPlane.GetSignedAltitude( testPoint );
		if ( signedAltitude > 0.0f )
		{
			isPointInside = false;
			break;
		}
...
//----------------------------------------------------------------------------------------------------------------------
void SwapValueOfTwoVariables( float& valueA, float& valueB )
{
...
//----------------------------------------------------------------------------------------------------------------------
void SwapValueOfTwoVariables( unsigned char& valueA, unsigned char& valueB )
{
	// TempVariable preserves value of variableA to enable swap between both variables A and B
...
//----------------------------------------------------------------------------------------------------------------------
RaycastResult2D RaycastVsPlane2D( Vec2 rayStartPos, Vec2 rayFwdNormal, float rayMaxLength, Plane2D const& plane )
{
	// Compute signedAltitude for rayEnd and rayStart
	// Raycast only hits if the ray "straddles" the plane
	Vec2  rayEnd							= rayStartPos + ( rayFwdNormal * rayMaxLength );
	float endProjectedLengthOnPlaneNormal	= DotProduct2D( rayEnd, plane.m_normal );
	float altitudeRayEnd					= endProjectedLengthOnPlaneNormal - plane.m_distFromOrigin; 
	float startProjectedLengthOnPlaneNormal = DotProduct2D( rayStartPos, plane.m_normal );
	float altitudeRayStart					= startProjectedLengthOnPlaneNormal - plane.m_distFromOrigin; 
	// If altitude of start and end are multiplied, a NEGATIVE result implies a raycast HIT (straddle)
	// If multiplied result is POSITIVE, raycast MISS
	float multipliedAltitudeResult			= altitudeRayStart * altitudeRayEnd;
	if ( multipliedAltitudeResult >= 0 )
	{
		RaycastResult2D rayMissResult = RaycastResult2D();
		return rayMissResult;
	}
	// Compute the impact point using similar triangles to solve for "impactDist"
	// Project rayFwd onto planeNormal 
	/*
	* rayFwdProjectedOntoNormal	   altitudeRayStart  
	* ------------------------- = ------------------
	*			  1				      impactDist
	*/
	float rayFwdProjectedOntoNormal = DotProduct2D( rayFwdNormal, plane.m_normal );
	// We have to negate it because why? It gives you the opposite signed dist?
	float impactDist				= -( altitudeRayStart / rayFwdProjectedOntoNormal );
	// Fill out rayHit info
	RaycastResult2D rayHitResult	= RaycastResult2D();
	rayHitResult.m_didImpact		= true;
	rayHitResult.m_impactDist		= impactDist;
	if ( altitudeRayStart > 0.0f )
	{
		// Ray hit from "above"
		rayHitResult.m_impactNormal	= plane.m_normal;
	}
	else
	{
		// Ray hit from "below"
	...
	rayHitResult.m_impactPos		= rayStartPos + ( rayFwdNormal * impactDist );
	rayHitResult.m_rayEndPos		= rayEnd;
	rayHitResult.m_rayForwardNormal	= rayFwdNormal;
	rayHitResult.m_rayMaxLength		= rayMaxLength;
	rayHitResult.m_rayStartPos		= rayStartPos;
	return rayHitResult;
}


//----------------------------------------------------------------------------------------------------------------------
RaycastResult2D RaycastVsConvexHull2D( Vec2 rayStartPos, Vec2 rayFwdNormal, float rayMaxLength, ConvexHull2D const& convexHull )
{
	// Raycast against all planes in the convexHull
	// Keep track of entry and exit points
	// Raycast only hits if the order is "lastEntry" then "firstExit"
	// Get midPointPos between "lastEntry" then "firstExit" and check IsPointInside
	std::vector<RaycastResult2D> entryPoints;
	std::vector<RaycastResult2D> exitPoints;
	std::vector<Plane2D> const& planeList		= convexHull.GetPlanesList();
	RaycastResult2D				rayHitResult	= RaycastResult2D();
	RaycastResult2D				rayMissResult	= RaycastResult2D();
	for ( int i = 0; i < planeList.size(); i++ )
	{
		Plane2D const& curPlane = planeList[i];
		rayHitResult = RaycastVsPlane2D( rayStartPos, rayFwdNormal, rayMaxLength, curPlane );
		if ( rayHitResult.m_didImpact )
		{
			float similarity = DotProduct2D( rayFwdNormal, curPlane.m_normal );
			if ( similarity < 0.0f )
			{
				// We disagree (ENTRY)
				entryPoints.push_back( rayHitResult );
			}
			else
			{
				// We agree (EXIT)
				exitPoints.push_back( rayHitResult );
			}
		}
	}
	//----------------------------------------------------------------------------------------------------------------------
	// Get lastEntry and firstExit by looping through list of 
	// entryPoints and exitPoints and sorting by impactDist
	//----------------------------------------------------------------------------------------------------------------------
	int entryPointIndex		= 0;
	float furthestEntryDist	= 0.0f;
	for ( int i = 0; i < entryPoints.size(); i++ )
	{
		if ( furthestEntryDist < entryPoints[ i ].m_impactDist )
		{
			entryPointIndex		= i;
			furthestEntryDist	= entryPoints[ i ].m_impactDist;
		}
	}
	int exitPointIndex	  = 0;
	float closestExitDist = 9999.9f;
	for ( int i = 0; i < exitPoints.size(); i++ )
	{
		if ( closestExitDist > exitPoints[ i ].m_impactDist )
		{
			exitPointIndex	= i;
			closestExitDist = exitPoints[ i ].m_impactDist;
		}
	}
	//----------------------------------------------------------------------------------------------------------------------
	// Handle edge case with 0 exitPoints
	//----------------------------------------------------------------------------------------------------------------------
	Vec2 rayEnd = rayStartPos + ( rayFwdNormal * rayMaxLength );
	if ( exitPoints.size() == 0 )
	{
		bool IsRayEndInsideConvexHull = IsPointInsideConvexHull2D( rayEnd, convexHull );
		if ( IsRayEndInsideConvexHull )
		{
			if ( entryPoints.size() == 0 )
			{
				// Edge case: rayStart AND rayEnd is inside ConvexHull
				RaycastResult2D rayInsideResult		= RaycastResult2D();
				rayInsideResult.m_didImpact			= true;
				rayInsideResult.m_impactDist		= 0.0f;
				rayInsideResult.m_impactNormal		= -rayFwdNormal;
				rayInsideResult.m_impactPos			= rayStartPos;
				rayInsideResult.m_rayEndPos			= rayEnd;
				rayInsideResult.m_rayForwardNormal	= rayFwdNormal;
				rayInsideResult.m_rayMaxLength		= rayMaxLength;
				rayInsideResult.m_rayStartPos		= rayStartPos;
				return rayInsideResult;
			}
			else
			{
				// Edge case: no exitPoints but entryPoints hit convexHull
	...
	if ( entryPoints.size() == 0 )
	{
		bool IsRayStartInsideConvexHull = IsPointInsideConvexHull2D( rayStartPos, convexHull );
		if ( IsRayStartInsideConvexHull )
		{
			// Edge case: rayStart is inside ConvexHull
			RaycastResult2D rayInsideResult		= RaycastResult2D();
			rayInsideResult.m_didImpact			= true;
			rayInsideResult.m_impactDist		= 0.0f;
			rayInsideResult.m_impactNormal		= -rayFwdNormal;
	...
	//----------------------------------------------------------------------------------------------------------------------
	// Get midPoint between lastEntry and firstExit
	//----------------------------------------------------------------------------------------------------------------------
	Vec2  lastEntry			= entryPoints[entryPointIndex].m_impactPos;
	Vec2  firstExit			=  exitPoints[exitPointIndex].m_impactPos;
	Vec2  fwdDirEntryToExit	= ( firstExit - lastEntry ).GetNormalized();
	float lengthEntryToExit = ( firstExit - lastEntry ).GetLength();
	lengthEntryToExit	   *= 0.5f;
	Vec2  midPoint			= lastEntry + ( fwdDirEntryToExit * lengthEntryToExit );
	// Check is point inside
	bool isPointInsideConvexHull = IsPointInsideConvexHull2D( midPoint, convexHull );
	if ( !isPointInsideConvexHull )
	{
		return rayMissResult;
	}
	return entryPoints[entryPointIndex];
...
RaycastResult3D RaycastVsAABB3D( Vec3 rayStartPos, Vec3 rayFwdNormal, float rayMaxLength, AABB3 aabb3 )
{
	...
	// Calculate 't' for minX, maxX, minY and maxY
	float rangeMinX = ( aabb3.m_mins.x - rayStartPos.x );
	float rangeMinY = ( aabb3.m_mins.y - rayStartPos.y );
	float rangeMinZ = ( aabb3.m_mins.z - rayStartPos.z );
	float rangeMaxX = ( aabb3.m_maxs.x - rayStartPos.x );
	float rangeMaxY = ( aabb3.m_maxs.y - rayStartPos.y );
	float rangeMaxZ = ( aabb3.m_maxs.z - rayStartPos.z );

	float timeMinX = rangeMinX / rayFwdNormal.x;
	float timeMinY = rangeMinY / rayFwdNormal.y;
	float timeMinZ = rangeMinZ / rayFwdNormal.z;
	float timeMaxX = rangeMaxX / rayFwdNormal.x;
	float timeMaxY = rangeMaxY / rayFwdNormal.y;
	float timeMaxZ = rangeMaxZ / rayFwdNormal.z;
	...
	// Determine hit result since X and Y ranges overlap
	bool xOverlappingY = floatRangeX.IsOverlapping( floatRangeY );		// XY
	bool xOverlappingZ = floatRangeX.IsOverlapping( floatRangeZ );		// XZ
	bool yOverlappingZ = floatRangeY.IsOverlapping( floatRangeZ );		// YZ
	if ( xOverlappingY && xOverlappingZ && yOverlappingZ )
	{
		raycastHitResult.m_didImpact = true;
		...
		else if ( CompareIfFloatsAreEqual( raycastHitResult.m_impactPos.z, aabb3.m_maxs.z, 0.1f ) )		// Top impact normal
		{
			Vec3 impactNormal = Vec3( 0.0f, 0.0f, 1.0f );
			raycastHitResult.m_impactNormal = impactNormal;
		}
...
//----------------------------------------------------------------------------------------------------------------------
RaycastResult3D RaycastVsOBB3D( Vec3 rayStartPos, Vec3 rayFwdNormal, float rayMaxLength, OBB3D obb3 )
{
	UNUSED( rayStartPos );
	UNUSED( rayFwdNormal );
	UNUSED( rayMaxLength );
...
//----------------------------------------------------------------------------------------------------------------------
Quaternion NormalizedLerp( float inValue, float inStart, float inEnd, Quaternion outStart, Quaternion outEnd )
{
	Quaternion quatOutStart = outStart;
	Quaternion quatOutEnd	= outEnd;

	// Ensure both quats are on the same hemisphere
	Vec4 outStart_V4 = quatOutStart.GetAsVec4();
	Vec4   outEnd_V4 =	 quatOutEnd.GetAsVec4();
	float dotResult  = DotProduct4D( outStart_V4, outEnd_V4 );
	if ( dotResult < 0.0f )
	{
		quatOutEnd = quatOutEnd * -1.0f;
	}

	// Lerp Quat start to end
	float fractionWithinRange	= GetFractionWithinRange( inValue, inStart, inEnd );
	Quaternion outRange			= quatOutEnd - quatOutStart;
	Quaternion result			= quatOutStart + ( outRange * fractionWithinRange );
	// Normalize a Quat
	result.Normalize();
...
//----------------------------------------------------------------------------------------------------------------------
Quaternion NormalizedLerp( Quaternion outStart, Quaternion outEnd, float parametricZeroToOne )
{
	Quaternion quatOutStart = outStart;
	Quaternion quatOutEnd	= outEnd;

	// Ensure both quats are on the same hemisphere to prevent lerping the "long way around"
	Vec4 outStart_V4 = quatOutStart.GetAsVec4();
	Vec4   outEnd_V4 =   quatOutEnd.GetAsVec4();
	float dotResult  = DotProduct4D( outStart_V4, outEnd_V4 );
	if ( dotResult < 0.0f )
	...
	Quaternion outRange = quatOutEnd - quatOutStart;
	Quaternion result	= quatOutStart + ( outRange * parametricZeroToOne );
	// Normalize a Quat
	result.Normalize();
...
//----------------------------------------------------------------------------------------------------------------------
float MoveTowardGoal( float currentValue, float goalValue, float maxDelta )
{
	float displacement = goalValue - currentValue;
	if ( fabsf( displacement ) <= maxDelta )
	{
...
{
	float fractionInValue			= GetFractionWithinRange( inValue, inStart, inEnd ) ;
	fractionInValue					= GetClampedZeroToOne( fractionInValue );
	float range						= outEnd - outStart;
	float pointOnRangeMinusStart	= range * fractionInValue;
	float pointOnRangePlusStart		= pointOnRangeMinusStart + outStart;
	return pointOnRangePlusStart;
}
...
//----------------------------------------------------------------------------------------------------------------------
// Normalizing vectors A and B makes the formula simpler
// Dot( A, B ) = A.GetLength() * B.GetLength() * Cos(theta)
// Dot( A, B ) = 1 * 1 * Cos(theta)
// Dot( A, B ) = Cos(theta)
// aCos( Dot(A, B) ) = theta
...
Vec3 RotateVectorAboutArbitraryAxis( Vec3 vectorToRotate, Vec3 arbitraryAxis, float degreesToTurn )
{
	...
	Vec3 c = ( 1.0f - CosDegrees( degreesToTurn ) ) * ( DotProduct3D( arbitraryAxis, vectorToRotate ) * arbitraryAxis ); 
	rotatedVector = a + b + c; 
