...
public:
	...
	static EulerAngles	GetAsEuler_XFwd_YLeft_ZUp( Quaternion const& quat );
	static EulerAngles	GetRelativeEulerAngles( EulerAngles const& childEuler, EulerAngles const& parentEuler );
	void TurnTowardGoal( EulerAngles const& eulerAnglesGoal, float deltaSeconds, float turnRateYaw, float turnRatePitch, float turnRateRoll );
