...
//----------------------------------------------------------------------------------------------------------------------
Vec3 const Vec3::MakeFromSphericalRadians( float yawRadians, float pitchRadians, float length, Vec2 const& startRadiusScalarXY )
{
	Vec3 vectorInCartesianCoords;
	vectorInCartesianCoords.x = ( cosf(yawRadians) * cosf(pitchRadians) ) * startRadiusScalarXY.x;
	vectorInCartesianCoords.y = ( sinf(yawRadians) * cosf(pitchRadians) ) * startRadiusScalarXY.y;
...
//----------------------------------------------------------------------------------------------------------------------
Vec3 const Vec3::MakeFromSphericalDegrees( float yawDegrees, float pitchDegrees, float length, Vec2 const& radiusScalarXY )
{
	float pitchRadians  = ConvertDegreesToRadians( pitchDegrees );
	float yawRadians	= ConvertDegreesToRadians( yawDegrees );

	Vec3 vectorInCartesianCoords;
	vectorInCartesianCoords = MakeFromSphericalRadians( yawRadians, pitchRadians, length, radiusScalarXY );
...
void Vec3::Normalize()
{
	 Vec3 currentVec3 = GetNormalized();
	 x = currentVec3.x;
	 y = currentVec3.y;
	 z = currentVec3.z;
}
...
void Vec3::SetLength(float newLength)
{
	...
	if ( CompareIfFloatsAreEqual( oldLength, 0.0f, 0.001f ) )
	{
		oldLength = 1.0f;
	}
